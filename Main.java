...
public class Main {
    public static void main(String[] args) {
        ...
        int lo = 0;
        int hi = arrLength;
        sortsTester(arrLength, lo, hi);

    }
    public static void sortsTester(int length, int lowerBound, int upperBound) {
        Integer[] arr = Tester.arrayGenerator(length, lowerBound, upperBound);

        Tester.selectionSort(arr);
        Tester.insertionSort(arr);
        Tester.shellSort(arr);
        Tester.elemMergeSort(arr);
        Tester.betterMergeSort(arr);
        Tester.elemQuickSort(arr);
        Tester.betterQuickSort(arr);
        Tester.quickSelection(arr);
        Tester.heapSort(arr);
    }
}
